...
export default function ReturnOrderScreen({ route }: any) {
  ...
  const submitReturn = async () => {
    ...
    // Đã bỏ validation bắt buộc ảnh với CUSTOMER
    // if (type === "CUSTOMER" && images.length === 0) {
    //   setNotify({
    //     visible: true,
    //     type: "error",
  ...
  // Cập nhật isValid - bỏ điều kiện bắt buộc ảnh với CUSTOMER
  const isValid = type !== null && reason.trim().length > 0;
  ...
  return (
    ...
    >
      ...
      {/* Hình ảnh minh chứng - Hiện cho cả 2 loại, không bắt buộc */}
      {type !== null && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
          ...
          <View style={styles.buttonRow}>
            ...
            <TouchableOpacity style={styles.actionBtn} onPress={pickImage}>
              <Ionicons name="image-outline" size={16} color="#06B6D4" />
          ...
          <Text style={styles.optionalHint}>
            Có thể thêm ảnh để minh chứng cho lý do hoàn đơn
          </Text>
        </View>
      )}
      ...
      <View style={styles.section}>
        ...
        <TextInput
          ...
          placeholder="Ví dụ: Khách không nghe điện thoại..."
          placeholderTextColor="#94A3B8"
          style={[styles.noteInput, showReasonError && styles.noteInputError]}
        />
        {showReasonError && (
