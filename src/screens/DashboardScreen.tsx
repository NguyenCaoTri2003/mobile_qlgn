...
//       },
//     });
//   }, [user, range, selectedDate, selectedMonth, selectedQuarter, selectedYear]);

//   useEffect(() => {
//     const subscription = AppState.addEventListener("change", (nextState) => {
//       if (nextState === "active") {
//         console.log("📱 App active -> reload");
...
export default function DashboardScreen({ navigation }: any) {
  ...
  // Dùng state để greeting tự động update khi quay lại app
  ...
  // Kiểm tra xem có phải ngày mới không và reset filter nếu cần
  ...
  //   // Nếu đang ở chế độ "day" và selectedDate khác hôm nay -> reset
  //   if (range === "day" && selectedDate !== todayStr) {
  //     console.log("🔄 Phát hiện ngày mới, reset filter...");
  //     resetFilter();
  //     return true;
  //   }
  ...
  const checkAndResetForNewDay = useCallback(() => {
    ...
    // Luôn reset filter về ngày hiện tại
    console.log("🔄 Reset filter về ngày hiện tại...");
    resetFilter();
    return true;
  }, [resetFilter]);
  ...
  //   const subscription = AppState.addEventListener("change", (nextState) => {
  //     if (nextState === "active") {
  //       console.log("📱 App active -> kiểm tra ngày mới và reload...");

  //       const didReset = checkAndResetForNewDay();

  //       if (didReset) {
  //         // Nếu đã reset filter, data sẽ tự động được fetch lại
  //         // thông qua useEffect phụ thuộc vào selectedDate
  //         console.log("✅ Đã reset filter cho ngày mới");
  //       } else {
  //         // Nếu không reset, vẫn reload data mới nhất
  //         refresh();
  //         if (isNVGN) refreshToday();
  //         if (user?.role === "QL" || user?.role === "SUPERADMIN") {
  //           fetchStats(buildParams());
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener("change", (nextState) => {
      if (nextState === "active") {
        console.log("📱 App active -> reset về ngày hiện tại...");

        // Luôn reset khi app active trở lại
        resetFilter();
        refresh();
        if (isNVGN) refreshToday();
        if (user?.role === "QL" || user?.role === "SUPERADMIN") {
  ...
  // Khi màn hình được focus (chuyển tab)
  // useFocusEffect(
  //   useCallback(() => {
  ...
  //     loadSettings();
  //     if (isNVGN) refreshToday();
  //   }, [isNVGN, checkAndResetForNewDay]),
  // );
  useFocusEffect(
    useCallback(() => {
      console.log("🎯 Screen focused -> reset về ngày hiện tại");
      resetFilter();
      loadSettings();
      if (isNVGN) refreshToday();
    }, [isNVGN, resetFilter]),
  ...
  // Auto-fetch stats khi params thay đổi
  useEffect(() => {
    if (user?.role === "QL" || user?.role === "SUPERADMIN") {
      fetchStats(buildParams());
    }
    ...
    <View style={{ flex: 1 }}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={["top", "left", "right"]}>
        ...
        >
          ...
          {/* TODAY ORDERS - Chỉ hiển thị cho NVGN */}
          {isNVGN && (
            <TodayOrdersCard orders={todayOrders} loading={todayLoading} />
          )}
